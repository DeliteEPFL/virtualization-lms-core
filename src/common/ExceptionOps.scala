...
trait ExceptionOps extends Variables {
  ...
  def fatal(m: Rep[String]): Rep[Nothing] = throw_exception(m)
  
  def throw_exception(m: Rep[String])(implicit pos: SourceContext): Rep[Nothing]
}

trait ExceptionOpsExp extends ExceptionOps with EffectExp {
  case class ThrowException(m: Rep[String]) extends Def[Nothing]
  
  def throw_exception(m: Exp[String])(implicit pos: SourceContext): Exp[Nothing] = reflectEffect(ThrowException(m), Global())(nothingTyp, pos)
