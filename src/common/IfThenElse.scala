...
trait IfThenElseExp extends IfThenElse with EffectExp {
  ...
  def __ifThenElseCopy[T:Typ](cond: Rep[Boolean], thenp: => Rep[T], elsep: => Rep[T], oldDef: Def[Any])(implicit pos: SourceContext): Exp[T] = {
    val a = reifyEffectsHere(thenp)
    val b = reifyEffectsHere(elsep)
...
trait IfThenElseExpOpt extends IfThenElseExp { this: BooleanOpsExp with EqualExpBridge =>
  ...
  override def __ifThenElseCopy[T:Typ](cond: Rep[Boolean], thenp: => Rep[T], elsep: => Rep[T], oldDef: Def[Any])(implicit pos: SourceContext) = cond match {
    case Const(true) => thenp
    case Const(false) => elsep
    case Def(BooleanNegate(a)) => __ifThenElseCopy(a, elsep, thenp, oldDef)
    case Def(e@NotEqual(a,b)) => __ifThenElseCopy(equals(a,b)(e.mA,e.mB,pos), elsep, thenp, oldDef)
    case _ =>
